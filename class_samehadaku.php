...
class SamehadakuClass{
	...
	public function getNewestAnime($page=1){
		...
		foreach($matches[1] as $m){
			...
			preg_match( '/src="(.*?)"/i', $m, $img );
			preg_match( '~Released on</b>: (.*?)</span>~', $m, $update );
			$link = $href[1];
			$slug = explode('/', $link);
			$slug = $slug[3];
			$title = $title[1];
			$img = $img[1];
			$update = $update[1];
			$animes[] = array('title'=>$title,'slug'=>$slug,'img'=>$img,'update'=>$update);
		}
		return $animes;
	}
	public function getAnimeDetails($slug){
		...
		$res = array('title'=>$title,'video_type'=>$list_jenis_video,'download_links'=>$download);
		return $res;
	}
